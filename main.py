...
class AutomatizadorApp:
    def __init__(self):
        ...
        # Conexion secundaria via UIA, SOLO para poder ver y clickear
        # los items del toolStrip1 (Adjuntar imagen / Ver imagen(es)).
        # Confirmado con diagnostico_toolstrip_uia.py: estos son
        # ToolStripButton sin HWND propio, por lo que el backend win32
        # (usado para todo lo demas: Edit, Static, Button normales de
        # los popups) nunca puede encontrarlos ni compararlos por clase.
    ...
    def conectar(self):
        ...
        # y misma clase, pero uno de los dos es un handle "fantasma"
        # (Windows lo sigue reportando pero ya no apunta a una ventana
        # real - usarlo da errores tipo "El identificador de la ventana
        # no es valido"). Por eso no alcanza con found_index=0 (a veces
        # elige el fantasma); filtramos explicitamente con
        # win32gui.IsWindow() y nos quedamos solo con el/los validos.
        ...
        # Mismo HWND, pero envuelto con backend uia (no es una conexion
        # nueva ni una ventana distinta - es la unica forma de que
        # pywinauto pueda "ver" los ToolStripButton de la barra de
        # herramientas, que no tienen HWND propio).
    ...
    def _campo_edit_por_etiqueta(self, texto_etiqueta: str):
        """Encuentra el control Edit asociado a una etiqueta (Static) de
        texto exacto, usando cercania geometrica (mismo renglon en
        pantalla, a la derecha de la etiqueta).

        Necesario porque en esta app los controles Edit no tienen texto
        propio (todos vienen con titulo vacio) - solo la etiqueta Static
        de al lado tiene el texto ("Nro:", "Nombre:", etc).

        IMPORTANTE: filtramos por clase manualmente en Python usando
        ctrl.class_name() (lee via GetClassName, confiable) en vez de
        pasar class_name_re a descendants(). Para el backend win32,
        descendants()/children() de pywinauto SOLO reconoce el kwarg
        'control_type' - cualquier otro kwarg (incluido class_name_re)
        se ignora en silencio y devuelve TODO sin filtrar. Ademas
        evitamos a proposito el filtro 'control_type' porque internamente
        usa RemoteMemoryBlock/SendMessage entre procesos, que es la
        misma via que fallo con AccessDenied al principio (posiblemente
        por el mismatch de bitness 32/64 bits que pywinauto advierte).
        """
        ...
        if etiqueta is None:
            raise RuntimeError(
                f"No se encontro ninguna etiqueta con texto {texto_etiqueta!r} "
                "en la ventana. Puede que el texto exacto sea distinto "
                "(revisa mayusculas/espacios) - correr diagnostico_campo_nro.py "
                "de nuevo para confirmar el texto actual."
            )
        ...
        for ctrl in todos:
            ...
            # mismo renglon (tolerancia vertical de 10px) y ubicado a la
            # derecha de la etiqueta (con pequeno margen por si se solapan)
        ...
        if mejor is None:
            raise RuntimeError(
                f"Se encontro la etiqueta {texto_etiqueta!r} pero ningun "
                "Edit en el mismo renglon. Puede que el layout sea "
                "distinto a lo esperado - avisa para ajustar la tolerancia."
            )
        return mejor
    ...
    def _buscar_boton(self, ventana, texto_boton: str):
        """Busca un boton por su texto exacto, filtrando por clase
        manualmente en Python con ctrl.class_name() (ver nota extensa
        en _campo_edit_por_etiqueta: descendants(class_name_re=...) no
        filtra nada para el backend win32, y control_type tampoco es
        confiable aqui)."""
        candidatos = ventana.descendants()
        for c in candidatos:
            try:
                if (c.class_name().startswith("WindowsForms10.BUTTON.")
                        and c.window_text().strip() == texto_boton):
                    return c
            except Exception:
                continue
        raise RuntimeError(
            f"No se encontro ningun boton con texto {texto_boton!r} en la "
            "ventana. Correr un diagnostico similar a diagnostico_campo_nro.py "
            "apuntando a esa ventana para confirmar el texto/clase exacta."
        )

    def _buscar_boton_toolstrip(self, texto_boton: str):
        """Busca un item del toolStrip1 (ToolStripButton) por su texto
        exacto, usando el backend UIA. Estos items NO tienen HWND
        propio, por lo que _buscar_boton (que filtra por clase
        'WindowsForms10.BUTTON.' via win32) nunca los encuentra -
        confirmado con diagnostico_toolstrip_uia.py."""
        for ctrl in self.ventana_principal_uia.descendants(control_type="Button"):
            try:
                ...
                continue
        raise RuntimeError(
            f"No se encontro el item de toolstrip con texto {texto_boton!r} "
            "via UIA. Correr diagnostico_toolstrip_uia.py de nuevo para "
            "confirmar el nombre exacto (cuidado con tildes)."
        )
    ...
    def contar_imagenes_adjuntas(self) -> int:
        ...
        boton_ver = self._buscar_boton_toolstrip(NOMBRE_BOTON_VER_IMAGENES)
        # click_input() simula un click real de mouse en las coordenadas
        # del control - mas confiable que click() para ToolStripButton,
        # que no siempre responde bien al patron Invoke de UIA.
        boton_ver.click_input()
        time.sleep(PAUSA_CORTA * 2)
        ...
        try:
            ...
            # OJO: el fallback original usaba control_type="Row", que
            # para backend win32 tampoco es un filtro valido (ver nota
            # en _buscar_boton) y hubiera devuelto siempre 0 filas de
            # forma silenciosa - un riesgo serio para la logica de
            # "no duplicar si ya tiene fotos". Si row_count() falla,
            # preferimos fallar fuerte en vez de asumir 0 en silencio.
            raise RuntimeError(
                "No se pudo leer la cantidad de filas de la tabla "
                "DataGridView (row_count() fallo). No se asume 0 para "
                "evitar subir fotos duplicadas por error - avisa con el "
                "traceback completo para ajustar esto."
            )

        # Cerrar la ventana de verificacion (boton "Cerrar" generico o Alt+F4)
        try:
            self._buscar_boton(ventana_ver, NOMBRE_BOTON_CERRAR_POPUP).click()
        except Exception:
            ventana_ver.close()
    ...
    def adjuntar_archivo(self, ruta_archivo: str):
        ...
        boton_adjuntar = self._buscar_boton_toolstrip(NOMBRE_BOTON_ADJUNTAR)
        boton_adjuntar.click_input()
        time.sleep(PAUSA_CORTA * 2)
