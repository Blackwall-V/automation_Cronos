...
class AutomatizadorApp:
    ...
    def _campo_edit_por_etiqueta(self, texto_etiqueta: str):
        ...
        de al lado tiene el texto ("Nro:", "Nombre:", etc). Ademas, el
        nombre de clase real de los Edit/Static incluye un sufijo tipo
        ".app.0.ea7f4a_r9_ad1" que es un hash de sesion y CAMBIA cada
        vez que se abre la app, asi que buscamos por prefijo de clase
        (class_name_re) en vez de por el nombre de clase exacto.
        """
        etiquetas = self.ventana_principal.descendants(
            class_name_re=r"^WindowsForms10\.STATIC\..*"
        )
        etiqueta = None
        for et in etiquetas:
            if et.window_text().strip() == texto_etiqueta:
                etiqueta = et
                break
        if etiqueta is None:
            raise RuntimeError(
                ...
                "(revisa mayusculas/espacios) - correr listar_controles_v2.py "
        ...
        candidatos = self.ventana_principal.descendants(
            class_name_re=r"^WindowsForms10\.EDIT\..*"
        )
        mejor = None
        mejor_dist = None
        for edit in candidatos:
            r = edit.rectangle()
    ...
    def _buscar_boton(self, ventana, texto_boton: str):
        """Busca un boton por su texto exacto usando class_name_re, NO
        control_type (control_type esta documentado en pywinauto como
        valido solo para el backend UIA - con backend win32, que es el
        que usamos aqui, ese filtro no aplica y siempre da 0 resultados,
        aunque el boton exista. Ver README / historial de conversacion
        si esto necesita ajustarse para otro tipo de control)."""
        candidatos = ventana.descendants(
            class_name_re=r"^WindowsForms10\.BUTTON\..*"
        )
        for c in candidatos:
            if c.window_text().strip() == texto_boton:
                return c
        raise RuntimeError(
            f"No se encontro ningun boton con texto {texto_boton!r} en la "
            "ventana. Correr listar_controles_v2.py apuntando a esa "
            "ventana para confirmar el texto/clase exacta."
        )
